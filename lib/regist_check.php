<?php
    ...
    function check_regist_form($id, $passwd, $passwd2, $name, $email, $comment){
        if(check_id($id) && check_name($name) && check_passwd($passwd) && check_retype_passwd($passwd, $passwd2) && check_email($email) && check_comment($comment)){
            return TRUE;
        }
        else
            return FALSE;
    ...
    function check_id($id){
        if(!ereg("[[:alnum:]+]{5,10}",$id)) {
            echo "<script>alert('5~10자리 영문,숫자만 허용됩니다.');</script>";
            return FALSE;
        }
        return TRUE;
    }
   ...
   //이름 유효성 검사
    ...
    function check_email($email){
        if(!ereg("(^[_0-9a-zA-Z-]+(\.[_0-9a-zA-Z-]+)*@[0-9a-zA-Z-]+(\.[0-9a-zA-Z-]+)*$)", $email)) {
            echo "<script>alert('올바른 email형식을 입력하세요');</script>";
            return FALSE;
        }
        return TRUE;
    }
    ...
    function check_comment($msg){
        if(mb_strlen($msg,"utf-8")<=30)
            return TRUE;
        echo "<script>alert('자기소개는 30자 이하로 입력하세요.');</script>";
        return FALSE;
    }
?>
